...
const dueTickets = computed(() => visibleTickets.value.filter((ticket) => !ticket.local_completed && ticket.planned_date && ticket.planned_date <= new Date().toISOString().slice(0, 10)));
...
async function addSubtask(ticket: Ticket, draft: { summary: string; description: string; planned_date: string }) { await run(async () => { await request(`/api/tickets/${ticket.id}/subtasks`, { method: "POST", body: JSON.stringify(draft) }); }, "Subtask added."); }
...
<template>
  <div class="shell">
    ...
    <main v-if="page === 'tickets'">
      ...
      <section class="ticket-section"><div class="section-heading"><div><span class="eyebrow">FOCUS</span><h2>Today</h2></div><span class="muted">Due today and overdue</span></div><div v-if="dueTickets.length" class="ticket-grid"><TicketCard v-for="ticket in dueTickets" :key="`due-${ticket.id}`" :ticket="ticket" :category-name="categoryName(ticket.category_id)" @toggle="toggle(`/api/tickets/${ticket.id}/complete`)" @sync="sync(`/api/tickets/${ticket.id}/sync`)" @remove="remove(`/api/tickets/${ticket.id}`)" @save="saveTicket" @save-subtask="saveSubtask" @toggle-subtask="toggle(`/api/subtasks/${$event}/complete`)" @remove-subtask="remove(`/api/subtasks/${$event}`)" /></div><div v-else class="empty-state"><i class="pi pi-check-circle"></i><strong>Nothing due right now</strong><span>Add the next useful thing when it arrives.</span></div></section>
      <section class="ticket-section"><div class="section-heading"><div><span class="eyebrow">QUEUE</span><h2>All tickets</h2></div><span class="muted">Drag-and-drop priority coming from the existing API</span></div><div v-if="visibleTickets.length" class="ticket-grid"><TicketCard v-for="ticket in visibleTickets" :key="ticket.id" :ticket="ticket" :category-name="categoryName(ticket.category_id)" @toggle="toggle(`/api/tickets/${ticket.id}/complete`)" @sync="sync(`/api/tickets/${ticket.id}/sync`)" @remove="remove(`/api/tickets/${ticket.id}`)" @save="saveTicket" @save-subtask="saveSubtask" @toggle-subtask="toggle(`/api/subtasks/${$event}/complete`)" @remove-subtask="remove(`/api/subtasks/${$event}`)" /></div><div v-else class="empty-state"><i class="pi pi-inbox"></i><strong>No tickets yet</strong><span>Create a ticket to start your queue.</span></div></section>
    </main>

    <main v-else-if="page === 'create'" class="narrow"><section class="hero"><div><span class="eyebrow">BUILD</span><h2>Create a ticket</h2><p>Capture the outcome and break it into clear next steps.</p></div></section><Card><template #content><form class="form" @submit.prevent="createTicket"><label>Summary or Jira reference<InputText v-model="newTicket.summary" required placeholder="What needs doing? Or WORK-123" /></label><div class="two-col"><label>Planned date<DatePicker v-model="newTicket.planned_date" dateFormat="yy-mm-dd" showIcon /></label><label>Category<Select v-model="newTicket.category_id" :options="state.categories" optionLabel="name" optionValue="id" placeholder="No category" /></label></div><label>Description<Textarea v-model="newTicket.description" rows="5" autoResize /></label><label>Jira import URL (optional)<InputText v-model="newTicket.jira_reference" placeholder="https://your-site/browse/WORK-123" /></label><div class="subtask-builder"><div class="section-heading"><h3>Subtasks</h3><Button type="button" label="Add step" icon="pi pi-plus" text @click="addDraftSubtask" /></div><div v-for="(subtask, index) in newSubtasks" :key="index" class="draft-row"><InputText v-model="subtask.summary" :placeholder="`Step ${index + 1}`" /><DatePicker v-model="subtask.planned_date" dateFormat="yy-mm-dd" showIcon /><Button type="button" icon="pi pi-trash" severity="danger" text @click="newSubtasks.splice(index, 1)" /></div></div><Button type="submit" label="Create ticket" icon="pi pi-check" :loading="busy" /></form></template></Card></main>
