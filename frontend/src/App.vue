...
interface Review {
  ...
  summary: string;
  description: string;
  merge_requests: ReviewMergeRequest[];
  selected_merge_request: ReviewMergeRequest | null;
  ready_to_merge_enabled: boolean;
  merge_request_selection_reason: string;
...
const reviewActionSuccesses = ref<Record<string, string>>({});
...
const newSubtasks = ref<Array<{ summary: string; description: string; planned_date: Date | null; component: string | null }>>([]);
...
async function saveSubtask(subtask: Ticket) { await run(() => request(`/api/subtasks/${subtask.id}`, { method: "PUT", body: JSON.stringify({ summary: subtask.summary, description: subtask.description, planned_date: subtask.planned_date, component: subtask.component }) }), "Subtask updated."); }
async function addSubtask(ticket: Ticket, draft: { summary: string; description: string; planned_date: string; component: string | null }) { await run(async () => { await request(`/api/tickets/${ticket.id}/subtasks`, { method: "POST", body: JSON.stringify({ ...draft, planned_date: draft.planned_date || null }) }); }, "Subtask added."); }
...
function addDraftSubtask() { newSubtasks.value.push({ summary: "", description: "", planned_date: null, component: null }); }
...
<template>
  <div class="shell" @drop="clearDragState" @dragend="clearDragState">
    ...
    <main v-if="page === 'tickets'">
      ...
      <section v-if="ticketView === 'focus'" id="focus-section" class="ticket-section"><div class="section-heading"><div><span class="eyebrow">FOCUS</span><h2>Today</h2></div><span class="muted">Due today and overdue</span></div><div v-if="dueTickets.length" class="ticket-grid"><TicketCard v-for="ticket in dueTickets" :key="`due-${ticket.id}`" :ticket="ticket" :categories="state.categories" :components="state.components" :browser-base-url="state.jira_config?.browser_base_url || ''" :category-name="categoryName(ticket.category_id)" @toggle="toggle(`/api/tickets/${ticket.id}/complete`)" @sync="sync(syncUrl(ticket))" @remove="remove(`/api/tickets/${ticket.id}`)" @save="saveTicket" @save-subtask="saveSubtask" @add-subtask="addSubtask" @toggle-subtask="toggle(`/api/subtasks/${$event}/complete`)" @remove-subtask="remove(`/api/subtasks/${$event}`)" @move-subtask="moveSubtask" /></div><div v-else class="empty-state"><i class="pi pi-check-circle"></i><strong>Nothing due right now</strong><span>Add the next useful thing when it arrives.</span></div></section>
        <section v-if="ticketView === 'queue'" id="queue-section" class="ticket-section"><div class="section-heading"><div><span class="eyebrow">QUEUE</span><h2>All tickets</h2></div><span class="muted">Drag active tickets to reorder.</span></div><div v-if="visibleTickets.length" class="ticket-grid"><TicketCard v-for="ticket in visibleTickets" :key="ticket.id" :ticket="ticket" :categories="state.categories" :components="state.components" :browser-base-url="state.jira_config?.browser_base_url || ''" :category-name="categoryName(ticket.category_id)" :reorderable="true" :dragging-ticket-id="draggingTicketId" :drag-over-ticket-id="dragOverTicketId" @ticket-drag-start="startTicketDrag" @ticket-drag-end="endTicketDrag" @ticket-drag-over="overTicket" @ticket-drop="dropTicket" @toggle="toggle(`/api/tickets/${ticket.id}/complete`)" @sync="sync(syncUrl(ticket))" @remove="remove(`/api/tickets/${ticket.id}`)" @save="saveTicket" @save-subtask="saveSubtask" @add-subtask="addSubtask" @toggle-subtask="toggle(`/api/subtasks/${$event}/complete`)" @remove-subtask="remove(`/api/subtasks/${$event}`)" @move-subtask="moveSubtask" /></div><div v-else class="empty-state"><i class="pi pi-inbox"></i><strong>No tickets yet</strong><span>Create a ticket to start your queue.</span></div></section>
     </main>
