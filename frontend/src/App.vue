...
import CategoryButtons, { type Category } from "./components/CategoryButtons.vue";
...
const newTicket = ref({ summary: "", description: "", notes: "", planned_date: null as Date | null, category_id: null as number | null, jira_reference: "" });
...
async function createTicket() {
  await run(async () => {
    ...
    newTicket.value = { summary: "", description: "", notes: "", planned_date: null, category_id: null, jira_reference: "" }; newSubtasks.value = [];
    go("tickets"); await load();
  }, "Ticket created.");
}
async function saveTicket(ticket: Ticket) { await run(() => request(`/api/tickets/${ticket.id}`, { method: "PUT", body: JSON.stringify({ summary: ticket.summary, description: ticket.description, notes: ticket.notes || "", planned_date: ticket.planned_date, category_id: ticket.category_id }) }), "Ticket updated."); }
